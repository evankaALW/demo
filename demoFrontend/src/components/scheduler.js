...
const Scheduler = () => {
  ...
  const [schedulerCount, setSchedulerCount] = useState(3);
  ...
  useEffect(() => {
    if(selectedTheater){//Fetch videos from the backend API using fetch
    ...
    const currentDate = new Date();// Set start dates for the next three days
...
const renderDropdowns = (startDate, schedulerIndex) => {
  return Array(15).fill().map((_, slotIndex) => (
    <div key={slotIndex} className="dropdown-container">
      ...
      <select value={selectedSchedules[schedulerIndex][slotIndex]} onChange={(e) => handleVideoChange(schedulerIndex, slotIndex, e.target.value)}>
        <option value="" disabled>Select a video</option>
        {getAvailableOptions(schedulerIndex, slotIndex).map(video => (
          <option key={video.videoID} value={video.videoURL}>{video.videoURL}</option>
        ))}
      </select>
    </div>
  ));
  ...
  const renderSchedulers = () => {
    if(videos.length===0){
      return(<h1>No data added in the slot page.</h1>);}
    else{
  ...
  const renderDropdown = () => {
    return (
      <div>
        <select className='selecttag' value={selectedTheater} onChange={(e) => setSelectedTheater(e.target.value)}>
        ...
        <select className='selecttag' value={schedulerCount} onChange={(e) => setSchedulerCount(parseInt(e.target.value, 10))}>
          {[3, 7, 15, 30].map((count) => (<option key={count} value={count}>{count}</option>))}
        </select>
      </div>
    )};
  return (
    <><h1>Slot Content</h1>
      {renderDropdown()}
      <div className="scheduler-wrapper">{renderSchedulers()}</div></>
  )};
